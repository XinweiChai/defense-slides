...
SLCG is \highlight{exact} for unreachability 

SLCG is \highlight{exact} when it does not contain self-dependent structure:
\begin{itemize}
...
\only<1>{When $a$ reaches level $1$, $b_1$ is no longer reachable as the condition of $\{a_0\}\to b_1$ is not satisfied.
$\to$ Reaching $a_1$ disables the reachability of $b_1$, \textit{vice versa}, $a_1$ is not reachable, \textit{i.e.} $c_1$ is unreachable}
\only<2->{$b_1$ is not blocked by $a$, $c_1$ is reachable \textit{via} $a_1::b_1::c_1$}
