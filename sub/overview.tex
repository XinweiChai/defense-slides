...
\begin{frame}{Problematics}
    \begin{tikzpicture}[line,>=stealth]
        \onslide<+->{\node [label=above:{Real system}] (realSys) { \includegraphics[scale=0.5]{realSystem.pdf}};
        }
        ...
        \onslide<+->{\node [color=blue,right = of bioNet,label=above:{Reachability}] (reach) {\includegraphics[scale=0.7]{reachability.pdf}};
